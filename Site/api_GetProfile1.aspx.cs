...
public partial class api_GetProfile1 : WOApiWebPage
{
    void AddChars(ref StringBuilder xml, bool FromServer)
    {
        ...
        while (reader.Read())
        {
            ...
            xml.Append(xml_attr("FriendlyFire", reader["FriendlyFire"]));
            xml.Append(xml_attr("ClanPoints", reader["ClanPoints"]));
    ...
    protected override void Execute()
    {
		...
		xml.Append(xml_attr("email", reader["email"]));
		xml.Append(xml_attr("username", reader["UserID"]));
        ...
        string IsMod = reader["IsMod"].ToString();
        if (IsMod != "0")
            xml.Append(xml_attr("IsMod", IsMod));
        ...
        int BanChatLeft = Convert.ToInt32(reader["BanChatLeft"]);
        if (BanChatLeft > 0)
            xml.Append(xml_attr("BanChatLeft", BanChatLeft));

        int BanChatCount = Convert.ToInt32(reader["BanChatCount"]);
        if (BanChatCount > 0)
            xml.Append(xml_attr("BanChatCount", BanChatCount));

        int XP50BoostsLeft = Convert.ToInt32(reader["XP50BoostsLeft"]);
        if (XP50BoostsLeft > 0)
            xml.Append(xml_attr("XP50BoostsLeft", XP50BoostsLeft));

        xml.Append(xml_attr("ChatSwearingCount", reader["ChatSwearingCount"]));
        xml.Append(xml_attr("RewardItemID", reader["RewardItemID"]));
        xml.Append(xml_attr("ShowDailyRewardMessage", reader["ShowDailyRewardMessage"])); 
