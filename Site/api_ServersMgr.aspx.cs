...
public partial class api_ServersMgr : WOApiWebPage
{
    ...
    static public void OutputXmlServerInfoPublic(StringBuilder xml, SqlDataReader reader)
    {
        xml.Append("<?xml version=\"1.0\"?>\n");
        xml.Append(string.Format("<slistpublic>"));

        while (reader.Read())
        {
            xml.Append("<srvpublic ");
            xml.Append(xml_attr("ID", reader["GameServerID"]));
            xml.Append(xml_attr("ServerRegion", reader["ServerRegion"]));
            xml.Append(xml_attr("ServerType", reader["ServerType"]));
            xml.Append(xml_attr("ServerFlags", reader["ServerFlags"]));
            ...
            xml.Append(xml_attr("ServerName", reader["ServerName"]));
            xml.Append(xml_attr("ServerPwd", reader["ServerPwd"]));
            xml.Append(xml_attr("Channel", reader["Channel"]));
            xml.Append(xml_attr("GameTimeLimit", reader["GameTimeLimit"]));
            xml.Append(xml_attr("Status", reader["Status"]));            
            xml.Append("/>");
        }
        xml.Append("</slistpublic>");
    }
    ...
    void SRV_ServerGetFullListPublic()
    {
        string skey1 = web.Param("skey1");
        if (skey1 != SERVER_API_KEY)
            throw new ApiExitException("bad key");
        ...
        SqlCommand sqcmd = new SqlCommand();
        sqcmd.CommandType = CommandType.StoredProcedure;
        sqcmd.CommandText = "WZ_ServerGetListPublic";

        if (!CallWOApi(sqcmd))
            return;

        StringBuilder xml = new StringBuilder();
        OutputXmlServerInfoPublic(xml, reader);
